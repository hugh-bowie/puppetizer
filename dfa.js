...
const { r, log, device, desktop, badAccounts, r15, r23 } = require('./src/helpers');
...
(async () => {
  try {
    ...
    //----login
    await page.goto('https://www.deerfieldagency.com/', { waitUntil: 'networkidle0' });
    const outboundLinks = await page.$$eval('a[href^="https:"]', out => out.map(al => al.getAttribute('href')));
    let allPageLinks = await page.$$eval('a[href^="/"]', next => next.map(nex => nex.getAttribute('href')));
    let uniquePageLinks = await [... new Set(allPageLinks)];
    console.log('outboundLinks: ' + outboundLinks);
    console.log('allPageLinks: ' + allPageLinks);
    console.log('uniquePageLinks: ' + uniquePageLinks);

    for (let l = 0; l < uniquePageLinks.length; l++) {
      await page.goto('https://www.deerfieldagency.com' + uniquePageLinks[l], { waitUntil: 'networkidle0' });
      await page.waitForTimeout(1500);
      await page.screenshot({ path: `./screenshots/${uniquePageLinks[l]}.png`, fullPage: 'true' });
      await page.waitForTimeout(1500);
    }
